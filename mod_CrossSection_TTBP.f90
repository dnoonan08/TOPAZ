...
integer :: NBin(1:NumMaxHisto),NHisto,npdf,nGluRad
...
do npdf=1,2
   ...
   do iHel=1,NumHelicities
      ...
      if( nGluRad.eq.1 ) then
           call TopDecay(ExtParticle(1),DK_1L_T,MomExt(1:4,6:8))
      elseif( TopDecays.EQ.4 .OR. TopDecays.EQ.2 ) then
...
do npdf=1,2
   ...
   do iHel=1,NumHelicities
      ...
      elseif( TopDecays.EQ.3 .OR. TopDecays.EQ.2 ) then
...
!print *, "remove nGlurad1";if(nglurad.eq.1) cycle
...
!print *, "remove npdf2";if(npdf.eq.2) cycle
...
!print *, "remove n+1 cuts"; applyPSCut=.false.
...
!print *, "remove n cuts"; applyPSCut=.false.
...
!-------------------------
!  print *, "inv ",MomExt(1,9)/m_top,(MomExt(1:4,9).dot.MomExt(1:4,7))/m_top**2,(MomExt(1:4,9).dot.MomExt(1:4,8))/m_top**2
!  print *, "medi",dble(LO_Res_Unpol),(dble(EvalCS_NLODK_ttbp)-dble(LO_Res_Unpol))
!  print *, "sub ",dble(LO_Res_Unpol)/(dble(EvalCS_NLODK_ttbp)-dble(LO_Res_Unpol)) + 1d0
!  pause
!  return
!-------------------------
...
!print *, "resetting EvalCS_NLODK_ttbp"; EvalCS_NLODK_ttbp=0d0
...
!print *, "remove n+1 cuts"; applyPSCut=.false.
   ...
   do GluHel=1,-1,-2 ! loop over additional gluon chiralities from decay
      ...
      if( nGluRad.eq.1 ) then
          ...
          call TopDecay(ExtParticle(2),DK_RE_Q,MomExt(1:4,9:12),GluonHel=GluHel)
      endif
...
!  normalization
   ...
   enddo
          ...
          do ndip=1,2!---there are two dipoles
              call wdec_trans(ndip,MomExt(1:4,9:12),MomExtTd(1:4,9:11),alpha_DKWff,dip_res_w)
              if( dip_res_w.eq.0d0 ) cycle
              Dipole = - alpha_s4Pi*RunFactor * CF * dip_res_w
...
!-------------------------
!  print *, "inv ",MomExt(1,12)/m_top,(MomExt(1:4,12).dot.MomExt(1:4,10))/m_top**2,(MomExt(1:4,12).dot.MomExt(1:4,11))/m_top**2
!  print *, "medi",dble(LO_Res_Unpol),(dble(EvalCS_NLODK_ttbp)-dble(LO_Res_Unpol))
!  print *, "sub ",dble(LO_Res_Unpol)/(dble(EvalCS_NLODK_ttbp)-dble(LO_Res_Unpol)) + 1d0
!  pause
!  return
...
include "vegas_common.f"
! SCHARFS STUFF
integer :: Top_Atop,i, count_t1, count_t2,ndip
real(8) :: dipole, galle, soft, coll
complex(8) :: test(1:4)
logical :: nan_t
!Dipole Stuff for contribution II.b
real(8) :: pbDpg,ptDpg,ptDpb, omz, rsq,z,y,dip_res_w
...
IF( CORRECTION.EQ.4 ) THEN
!----------------------------------------------------------
! 1. photon emission off anti-top  with one loop correction   |
!----------------------------------------------------------
...
!----------------------------------------------------------------
! 2. photon emission off top-decay    with one loop correction   |
!----------------------------------------------------------------
...
!------------------------------------------------------------------
! 3. photon emission off W-  with one loop correction  at anti-top |
!------------------------------------------------------------------
...
!-------------------------------------------------------------
! 4. photon emission off W+  with one loop correction  at top |
!-------------------------------------------------------------
...
!-------------------------------------------------------------------
! 5. photon emission off anti-top  with one loop correction  at top |
!-------------------------------------------------------------------
...
!-------------------------------------------------------------------
! 6. photon emission off top  with one loop correction  at anti-top |
!-------------------------------------------------------------------
...
!-----------------------------------------------------------------
! 7. photon emission off anti-top with one loop correction on W-  |
!-----------------------------------------------------------------
      ...
      do iHel=1,NumHelicities ! loop over initial state chiralities
         do PhoHel=1,-1,-2 ! loop over additional photon polarization
            ...
            do iPrimAmp=1,NumBornAmps
               ...
               TreeResult(iPrimAmp) = BornAmps(iPrimAmp)%Result
            enddo
            call TopDecay(ExtParticle(1),DKP_1LQ_T,MomExt(1:4,5:8),PhotonHel=PhoHel)
            call TopDecay(ExtParticle(2),DK_LO,MomExt(1:4,9:11))
            do iPrimAmp=1,NumBornAmps
               call EvalTree(BornAmps(iPrimAmp))
               DKResult(iPrimAmp) = BornAmps(iPrimAmp)%Result
            ...
            do jPrimAmp=1,NumBornAmps
               do iPrimAmp=1,NumBornAmps
                  NLO_Res_Pol = NLO_Res_Pol + ColCorrLO(iPrimAmp,jPrimAmp) * ParityFlip*dreal(TreeResult(jPrimAmp)*dconjg(DKResult(iPrimAmp)))
               enddo
            enddo
            NLO_Res_UnPol = NLO_Res_UnPol + NLO_Res_Pol
         enddo!helicity loop
...
!------------------------------------------------------------------
! 8. photon emission off top-decay with one loop correction on W+  |
!------------------------------------------------------------------
if( TOPDECAYS.eq.3 .OR. TOPDECAYS.eq.2 ) then 
   ...
   do PhoHel=1,-1,-2 ! loop over additional photon chiralities
      ...
      call TopDecay(ExtParticle(1),DK_LO,MomExt(1:4,5:7))
      call TopDecay(ExtParticle(2),DKP_1LQ_T,MomExt(1:4,8:11),PhotonHel=PhoHel)
      do iPrimAmp=1,NumBornAmps
         call EvalTree(BornAmps(iPrimAmp))
         DKResult(iPrimAmp) = BornAmps(iPrimAmp)%Result
      enddo
...
IF( TOPDECAYS.EQ.3 .OR. TOPDECAYS.EQ.2 ) THEN
!-----------------------------------------------------------------------------
! 9. photon emission off anti-top and W- with one loop correction at W+       |
!-----------------------------------------------------------------------------
...
IF( TOPDECAYS.EQ.4 .OR. TOPDECAYS.EQ.2 ) THEN
!---------------------------------------------------------------------
! 10. photon emission off top and W+ with one loop correction at W-   |
!---------------------------------------------------------------------
...
!                               nPhoRad=2: photon radiation off W/lep
   ...
   do PhoHel=1,-1,-2 ! loop over additional photon chiralities
      ...
      ! 2nd CALL FOR INTERFERENCE TERMS
      call TopDecay(ExtParticle(1),DK_1L_Q,MomExt(1:4,5:7))
      if( nPhoRad.eq.1 ) then
         call TopDecay(ExtParticle(2),DKP_LO_T,MomExt(1:4,8:11),PhotonHel=PhoHel)
      else
         call TopDecay(ExtParticle(2),DKP_LO_L,MomExt(1:4,8:11),PhotonHel=PhoHel)
      ...
      do jPrimAmp=1,NumBornAmps
         do iPrimAmp=1,NumBornAmps
            NLO_Res_Pol = NLO_Res_Pol + ColCorrLO(iPrimAmp,jPrimAmp) * ParityFlip*dreal(TreeResult(iPrimAmp)*dconjg(DKResult(jPrimAmp)))
         enddo
      enddo
      NLO_Res_UnPol = NLO_Res_UnPol + NLO_Res_Pol
...
ELSEIF (CORRECTION .eq. 5) then
!--------------------------------------------------------
! 1. photon emission off anti-top  with real correction  |
!--------------------------------------------------------
...
! print *, "sij",(MomExt(1:4,8).dot.MomExt(1:4,5))/m_top**2,MomExt(1,8)/m_top
! print *, "real",dble(NLO_Res_Unpol)

!------------------------------------------------------
! 1. photon emission off anti-top dipole -contribution |
!------------------------------------------------------
...
!---------------------------------------------------
! 2. photon emission off top  with real correction  |
!---------------------------------------------------
      call EvalPhasespace_TopDecay(MomExt(1:4,3),yRnd(5:8),.false.,MomExt(1:4,5:7),PSWgt2)
...
1199 continue
!--------------------------------------------------
! 2. photon emission off top dipole-contribution   |
!--------------------------------------------------
      MomExtTilde = MomExt
      Dip_Res_Unpol = (0.d0,0.d0)
      Top_Atop =1
...
!--------------------------------------------------------------
! 3. photon emission off W-  with real correction  at anti-top |
!--------------------------------------------------------------
...
!----------------------------------------------------------------------------------
! 3. photon emission off W-, anti-top dipole-contribution for tb-> bb W- gluon     |
!----------------------------------------------------------------------------------
      MomExtTilde = MomExt
...
!------------------------------------------------------------------
! 4. photon emission off W+  with real correction  at the top quark|
!------------------------------------------------------------------
...
188 continue
!---------------------------------------------------------------------
! 4. photon emission off W+ with dipole-contribution  at the top quark|
!---------------------------------------------------------------------
      MomExtTilde = MomExt
...
!--------------------------------------------------------------
! 5. photon emission off anti-top  and real correction on top  |
!--------------------------------------------------------------
...
177  continue
!-------------------------------------------------------------------------
! 5. photon emission off anti-top, dipole-contribution for t-> b W+ gluon |
!-------------------------------------------------------------------------
      MomExtTilde = MomExt
      MomExtTilde(1:4,12) = 0.d0
...
!-----------------------------------------------------------------------
! 6. photon emission off top  with real correction  at the antitop quark|
!-----------------------------------------------------------------------
 do nPhoRad=nPhoRad1,nPhoRad2!   nPhoRad=1: photon radiation off top/bot/W,
                             !   nPhoRad=2: photon radiation off W/lep
    if( nPhoRad.eq.1 ) then
    ...
    call CheckSing(MomExt(1:4,5:8),applySingCut)
    if(applySingcut .or. PSWgt2 .eq. 0.d0 .or. PSWgt3 .eq. 0.d0 ) goto 1880
...
7711 continue
!----------------------------------------------------------------------------
! 6. photon emission off top with dipole-contribution  at the antitop quark  |
!----------------------------------------------------------------------------
      MomExtTilde = MomExt
...
!------------------------------------------------------------
! 7. photon emission off anti-top with real correction at W- |
!------------------------------------------------------------
if( TOPDECAYS.eq.4 .OR. TOPDECAYS.eq.2 ) then 
  call EvalPhasespace_TopDecay4(MomExt(1:4,3),yRnd(5:14),MomExt(1:4,5:9),PSWgt2)  ! 5:b, 6:l, 7:nu, 8:glu(from W), 9:pho(from t) 
  call EvalPhasespace_TopDecay(MomExt(1:4,4),yRnd(15:18),.false.,MomExt(1:4,10:12),PSWgt3)
  PreFac = fbGeV2 * FluxFac * sHatJacobi * PSWgt1*PSWgt2*PSWgt3 * VgsWgt * PDFFac
      ...
      do iHel=1,NumHelicities ! loop over initial state chiralities
         do PhoHel=1,-1,-2 ! loop over additional photon chiralities
            do GluHel=1,-1,-2 ! loop over additional real gluon chiralities
               call HelCrossing(Helicities(iHel,1:NumExtParticles))
               call SetPolarizations()
               call TopDecay(ExtParticle(1),DKP_REQ_T,MomExt(1:4,5:9),GluonHel=GluHel,PhotonHel=PhoHel)
               call TopDecay(ExtParticle(2),DK_LO,MomExt(1:4,10:12))
               do iPrimAmp=1,NumBornAmps
                  call EvalTree(BornAmps(iPrimAmp))
                  DKResult(iPrimAmp) = BornAmps(iPrimAmp)%Result
               ...
               do jPrimAmp=1,NumBornAmps
                  do iPrimAmp=1,NumBornAmps
                     NLO_Res_Pol = NLO_Res_Pol + ColCorrLO(iPrimAmp,jPrimAmp) * ParityFlip*DKResult(iPrimAmp)*dconjg(DKResult(jPrimAmp))
                  enddo
               enddo
               NLO_Res_UnPol = NLO_Res_UnPol + NLO_Res_Pol
            enddo
...
! print *, (MomExt(1:4,6).dot.MomExt(1:4,8))/m_top**2, (MomExt(1:4,7).dot.MomExt(1:4,8))/m_top**2
! print *, dble(NLO_Res_Unpol)


!---------------------------------------
! 7. dipole subtraction for W- decay    |
!---------------------------------------
          do ndip=1,2!---there are two dipoles
              call wdec_trans(ndip,MomExt(1:4,5:8),MomExtTilde(1:4,5:7),alpha_DKWff,dip_res_w)
                           
              if( dip_res_w.eq.0d0 ) cycle
              Dipole = - alpha_s4Pi*RunFactor * CF * dip_res_w

              MomExtTilde(1:4,1:4)  = MomExt(1:4,1:4)
              MomExtTilde(1:4,8:11) = MomExt(1:4,9:12)
              call Kinematics_TTBARPHOTON(0,MomExtTilde(1:4,1:12),(/3,4,8,1,2,0,5,6,7,9,10,11/),applyPSCut,NBin)             
              if( applyPSCut ) cycle
              Dip_Res_Unpol= (0d0,0d0)
              do iHel=1,NumHelicities ! loop over initial state chiralities
              do PhoHel=1,-1,-2 ! loop over additional photon polarization
                  call HelCrossing(Helicities(iHel,1:NumExtParticles))
                  call SetPolarizations()
                  call TopDecay(ExtParticle(1),DKP_LO_T,MomExtTilde(1:4,5:8),PhotonHel=PhoHel)
                  call TopDecay(ExtParticle(2),DK_LO,MomExtTilde(1:4,9:11))
                  do iPrimAmp=1,NumBornAmps
                     call EvalTree(BornAmps(iPrimAmp))
                     DKResult(iPrimAmp) = BornAmps(iPrimAmp)%Result
                  enddo
                  ...
                  do jPrimAmp=1,NumBornAmps
                     do iPrimAmp=1,NumBornAmps
                        LO_Res_Pol = LO_Res_Pol + ColCorrLO(iPrimAmp,jPrimAmp) * ParityFlip*DKResult(iPrimAmp)*dconjg(DKResult(jPrimAmp))
                     enddo
                  enddo
                  Dip_Res_UnPol = Dip_Res_UnPol + LO_Res_Pol
              enddo!helicity loop
...
! print *, "my eik", dble(Dip_Res_UnPol)* ISFac * (alpha_s4Pi*RunFactor)**2 * PreFac * alpha_s4Pi*RunFactor * CF * &
!                                           (2*(MomExt(1:4,6).dot.MomExt(1:4,7))/(MomExt(1:4,6).dot.MomExt(1:4,8))/(MomExt(1:4,7).dot.MomExt(1:4,8))  )
              
!  normalization          
            Dip_Res_UnPol = Dip_Res_UnPol* ISFac * (alpha_s4Pi*RunFactor)**2 * PreFac * (Dipole)
! print *, ndip,  dble(Dip_Res_UnPol)
            EvalCS_NLODKP_ttb = EvalCS_NLODKP_ttb + Dip_Res_Unpol
...
!-------------------------------------------------------
! 8. photon emission off top with real correction at W+ |
!-------------------------------------------------------
if( TOPDECAYS.eq.3 .OR. TOPDECAYS.eq.2 ) then 
   call EvalPhasespace_TopDecay4(MomExt(1:4,4),yRnd(5:14),MomExt(1:4,8:12),PSWgt2)
   call EvalPhasespace_TopDecay(MomExt(1:4,3),yRnd(15:18),.false.,MomExt(1:4,5:7),PSWgt3)! 8:b, 9:l, 10:nu, 11:glu(from W), 12:pho(from tb)
   PreFac = fbGeV2 * FluxFac * sHatJacobi * PSWgt1*PSWgt2*PSWgt3 * VgsWgt * PDFFac
      ...
      do iHel=1,NumHelicities ! loop over initial state chiralities
         do PhoHel=1,-1,-2 ! loop over additional photon chiralities
            do GluHel=1,-1,-2 ! loop over additional real gluon chiralities
               call HelCrossing(Helicities(iHel,1:NumExtParticles))
               call SetPolarizations()
               call TopDecay(ExtParticle(2),DKP_REQ_T,MomExt(1:4,8:12),GluonHel=GluHel,PhotonHel=PhoHel)
               call TopDecay(ExtParticle(1),DK_LO,MomExt(1:4,5:7))
               do iPrimAmp=1,NumBornAmps
                  call EvalTree(BornAmps(iPrimAmp))
                  DKResult(iPrimAmp) = BornAmps(iPrimAmp)%Result
...
! print *, (MomExt(1:4,9).dot.MomExt(1:4,11))/m_top**2, (MomExt(1:4,10).dot.MomExt(1:4,11))/m_top**2
! print *, dble(NLO_Res_Unpol)

!---------------------------------------
! 8. dipole subtraction for W+ decay    |
!---------------------------------------
          do ndip=1,2!---there are two dipoles
              call wdec_trans(ndip,MomExt(1:4,8:11),MomExtTilde(1:4,9:11),alpha_DKWff,dip_res_w)
              if( dip_res_w.eq.0d0 ) cycle
              Dipole = - alpha_s4Pi*RunFactor * CF * dip_res_w

              MomExtTilde(1:4,1:7)= MomExt(1:4,1:7)
              MomExtTilde(1:4,12) = MomExt(1:4,12)
              call Kinematics_TTBARPHOTON(0,MomExtTilde(1:4,1:12),(/3,4,12,1,2,0,5,6,7,9,10,11/),applyPSCut,NBin)
              ...
              do PhoHel=1,-1,-2 ! loop over additional photon polarization
                  call HelCrossing(Helicities(iHel,1:NumExtParticles))
                  call SetPolarizations()
                  call TopDecay(ExtParticle(2),DKP_LO_T,MomExtTilde(1:4,9:12),PhotonHel=PhoHel)
                  call TopDecay(ExtParticle(1),DK_LO,MomExt(1:4,5:7))                                    
                  do iPrimAmp=1,NumBornAmps
                     call EvalTree(BornAmps(iPrimAmp))
                     DKResult(iPrimAmp) = BornAmps(iPrimAmp)%Result
...
! print *, "my eik", dble(Dip_Res_UnPol)* ISFac * (alpha_s4Pi*RunFactor)**2 * PreFac * alpha_s4Pi*RunFactor * CF * &
!                                           (2*(MomExt(1:4,9).dot.MomExt(1:4,10))/(MomExt(1:4,9).dot.MomExt(1:4,11))/(MomExt(1:4,10).dot.MomExt(1:4,11))  )
...
IF( TOPDECAYS.EQ.3 .OR. TOPDECAYS.EQ.2 ) THEN
!----------------------------------------------------------------------
! 9. photon emission off anti-top and W-  and real correction on W+    |
!----------------------------------------------------------------------
 do nPhoRad=nPhoRad1,nPhoRad2!   nPhoRad=1: photon radiation off top/bot/W,
 !                               nPhoRad=2: photon radiation off W/lep
    if( nPhoRad.eq.1 ) then
    ...
    call EvalPhasespace_TopDecay2(MomExt(1:4,4),yRnd(12:18),.true.,MomExt(1:4,9:12),PSWgt3)    
    PreFac = fbGeV2 * FluxFac * sHatJacobi * PSWgt1*PSWgt2*PSWgt3 * VgsWgt * PDFFac

    call CheckSing(MomExt(1:4,9:12),applySingCut)
    if( applySingcut .or. PSWgt2 .eq.0d0 .or. PSWgt3.eq.0d0 ) goto 7778

    call Kinematics_TTBARPHOTON(1,MomExt(1:4,1:12),(/3,4,8,1,2,12,5,6,7,9,10,11/),applyPSCut,NBin)
    if( applyPSCut ) goto 178
      ...
      NLO_Res_Unpol = NLO_Res_Unpol * ISFac * (alpha_s4Pi*RunFactor)**2 * PreFac
      EvalCS_NLODKP_ttb = EvalCS_NLODKP_ttb + dble(NLO_Res_Unpol)
! print *, "nPhoRad",nPhoRad
! print *, "sing", (MomExt(1:4,10).dot.MomExt(1:4,12))/m_W**2,(MomExt(1:4,11).dot.MomExt(1:4,12))/m_W**2
! print *, "real",dble(NLO_Res_Unpol)
...
178  continue
!------------------------------------------------
!  9. dipole-contribution for W+ decay           |
!------------------------------------------------
     do ndip=1,2!---there are two dipoles
          call wdec_trans(ndip,MomExt(1:4,9:12),MomExtTilde(1:4,9:11),alpha_DKWff,dip_res_w)
                           
          if( dip_res_w.eq.0d0 ) cycle
          Dipole = - alpha_s4Pi*RunFactor * CF * dip_res_w

          MomExtTilde(1:4,1:8) = MomExt(1:4,1:8)
          call Kinematics_TTBARPHOTON(0,MomExtTilde(1:4,1:12),(/3,4,8,1,2,0,5,6,7,9,10,11/),applyPSCut,NBin)
          if( applyPSCut ) cycle
          ...
          Dip_Res_UnPol = Dip_Res_UnPol* ISFac * (alpha_s4Pi*RunFactor)**2 * PreFac * (dipole)
          EvalCS_NLODKP_ttb = EvalCS_NLODKP_ttb + dble(Dip_Res_Unpol)
! print *, "dipo", ndip,   dble(Dip_Res_Unpol)           
...
IF( TOPDECAYS.EQ.4 .OR. TOPDECAYS.EQ.2 ) THEN
!-----------------------------------------------------------------------
! 10. photon emission off top and W- and real correction on W-          |
!-----------------------------------------------------------------------
 do nPhoRad=nPhoRad1,nPhoRad2!   nPhoRad=1: photon radiation off top/bot/W,
                             !   nPhoRad=2: photon radiation off W/lep
    if( nPhoRad.eq.1 ) then
       call EvalPhasespace_TopDecay(MomExt(1:4,4),yRnd(12:18),.true.,MomExt(1:4,9:12),PSWgt2) 
    else
       call EvalPhasespace_TopDecay2(MomExt(1:4,4),yRnd(12:18),.true.,MomExt(1:4,9:12),PSWgt2)
    endif
    call EvalPhasespace_TopDecay2(MomExt(1:4,3),yRnd(5:11),.true.,MomExt(1:4,5:8),PSWgt3)    

    call CheckSing(MomExt(1:4,5:8),applySingCut)
    if( applySingcut .or. PSWgt2.eq.0d0 .or. PSWgt3.eq.00) goto 7713

    PreFac = fbGeV2 * FluxFac * sHatJacobi * PSWgt1*PSWgt2*PSWgt3 * VgsWgt * PDFFac
    call Kinematics_TTBARPHOTON(1,MomExt(1:4,1:12),(/3,4,12,1,2,8,5,6,7,9,10,11/),applyPSCut,NBin)
    if( applyPSCut ) goto 7712
      ...
      do iHel=1,NumHelicities ! loop over initial state chiralities
         do PhoHel=1,-1,-2 ! loop over additional photon chiralities
            do GluHel=1,-1,-2 ! loop over additional real gluon chiralities
               ...
               call TopDecay(ExtParticle(1),DK_RE_Q,MomExt(1:4,5:8),GluonHel=GluHel)
...
! print *, "nPhoRad",nPhoRad
! print *, "sing", (MomExt(1:4,6).dot.MomExt(1:4,8))/m_W**2,(MomExt(1:4,7).dot.MomExt(1:4,8))/m_W**2
! print *, "real",dble(NLO_Res_Unpol)      
...
!----------------------------------
          do ndip=1,2!---there are two dipoles
              ...
              MomExtTilde(1:4,1:4)= MomExt(1:4,1:4)
              MomExtTilde(1:4,9:12) = MomExt(1:4,9:12)
              call Kinematics_TTBARPHOTON(0,MomExtTilde(1:4,1:12),(/3,4,12,1,2,0,5,6,7,9,10,11/),applyPSCut,NBin)
              if (applyPScut) cycle
              ...
              do iHel=1,NumHelicities ! loop over initial state chiralities
                 do PhoHel=1,-1,-2 ! loop over additional photon chiralities
                    ...
                    if(nPhoRad .eq. 1) then
                       ...
                       call TopDecay(ExtParticle(2),DKP_LO_L,MomExtTilde(1:4,9:12),PhotonHel=PhoHel)
                    endif
                    call TopDecay(ExtParticle(1),DK_LO,MomExtTilde(1:4,5:7))
